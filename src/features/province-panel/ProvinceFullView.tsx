...
function Fila({ index, style, items, seleccionadoId, onSelect }: RowComponentProps<RowProps>) {
    ...
    <div style={style} className="px-3 py-0.5">
      <button
        ...
        className={`flex w-full items-center gap-3 rounded-lg border px-3 py-2 text-left transition-colors ${
          activo
            ? 'border-[#f5820d] bg-[#f5820d]/10'
            : 'border-transparent hover:bg-neutral-900'
        }`}
      >
        <Icono className="h-4 w-4 shrink-0 text-neutral-500" aria-hidden="true" />
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm text-neutral-100">{nombreEfectivo(espacio)}</div>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-start gap-3">
        <Icono className="h-8 w-8 shrink-0 text-[#f5820d]" aria-hidden="true" />
        <div>
          <h3 className="text-2xl font-semibold text-neutral-100">{nombreEfectivo(espacio)}</h3>
          <p className="font-mono text-sm uppercase tracking-wide text-neutral-500">
            {espacio.categoria}
            {espacio.subcategoria &&
            espacio.subcategoria.toLowerCase() !== espacio.categoria.toLowerCase()
              ? ` · ${espacio.subcategoria}`
              : ''}
          </p>
        </div>
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-4 font-mono text-sm">
        <div>
          <dt className="text-xs uppercase tracking-wide text-neutral-500">Año</dt>
          <dd className="text-neutral-200">{espacio.anioInauguracion ?? 's/d'}</dd>
        </div>
        <div>
          <dt className="text-xs uppercase tracking-wide text-neutral-500">Gestión</dt>
          <dd className="text-neutral-200 capitalize">{espacio.gestion ?? 's/d'}</dd>
        </div>
        <div className="col-span-2">
          <dt className="text-xs uppercase tracking-wide text-neutral-500">Localidad</dt>
          <dd className="text-neutral-200">
            {[espacio.localidad, espacio.departamento].filter(Boolean).join(', ') || 's/d'}
          </dd>
        </div>
        {espacio.direccion && (
          <div className="col-span-2">
      ...
      {(espacio.telefono || espacio.mail || espacio.web) && (
        <div className="flex flex-col gap-1.5 border-t border-neutral-800 pt-4 text-base">
          {espacio.telefono && <div className="text-neutral-300">☎ {espacio.telefono}</div>}
          {espacio.mail && (
            <a href={`mailto:${espacio.mail}`} className="text-[#f5820d] hover:underline">
              {espacio.mail}
            </a>
          )}
          {espacio.web && (
            <a
              href={espacio.web.startsWith('http') ? espacio.web : `https://${espacio.web}`}
              target="_blank"
              rel="noreferrer"
              className="truncate text-[#f5820d] hover:underline"
            >
              {espacio.web}
            </a>
          )}
...
function ProvinceFullViewContent({
  ...
  const categorias = useMemo(() => {
    ...
    for (const e of espacios) conteo.set(e.categoria, (conteo.get(e.categoria) ?? 0) + 1)
  ...
  const filtrados = useMemo(() => {
    ...
    if (gestionesActivas) {
      resultado = resultado.filter((e) => gestionesActivas.has(e.gestion ?? 'sin dato'))
    }
    if (localidadActiva) {
      resultado = resultado.filter((e) => (e.localidad ?? 'sin dato') === localidadActiva)
    }
    ...
    >
      ...
      {!espacios ? (
        ...
        <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
          <div className="flex w-full min-w-0 flex-col gap-3 border-b border-neutral-800 p-4 md:w-[380px] md:flex-shrink-0 md:border-b-0 md:border-r">
            <input
              type="search"
              value={busqueda}
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <div className="flex gap-2 font-mono text-[10px] uppercase tracking-wide text-neutral-500">
                  <button type="button" onClick={() => setCategoriasActivas(null)} className="hover:text-neutral-200">
                    Todas
                  </button>
                  <span aria-hidden="true">·</span>
                  <button type="button" onClick={() => setCategoriasActivas(new Set())} className="hover:text-neutral-200">
                    Ninguna
                  </button>
                </div>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {categorias.map(([categoria, count]) => {
                  const activa = categoriasActivas ? categoriasActivas.has(categoria) : true
                  return (
                    <button
                      key={categoria}
                      type="button"
                      ...
                      className={`rounded-full border px-2.5 py-1 text-xs transition-colors ${
                        activa
                          ? 'border-[#f5820d] bg-[#f5820d]/15 text-[#f5820d]'
            ...
            {gestiones.length > 1 && (
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  <div className="flex gap-2 font-mono text-[10px] uppercase tracking-wide text-neutral-500">
                    <button type="button" onClick={() => setGestionesActivas(null)} className="hover:text-neutral-200">
                      Todas
                    </button>
                    <span aria-hidden="true">·</span>
                    <button type="button" onClick={() => setGestionesActivas(new Set())} className="hover:text-neutral-200">
                      Ninguna
                    </button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {gestiones.map(([gestion, count]) => {
                    const activa = gestionesActivas ? gestionesActivas.has(gestion) : true
                    return (
                      <button
                        key={gestion}
                        type="button"
